...
class RoomViewController: UIViewController{
    ...
    override func viewDidLoad() {
        ...
        let enemyName = userDefault.string(forKey: "enemyName")
        RoomID = userDefault.string(forKey: "ルームID")
        ...
        self.ref.child("rooms").child(String(RoomID)).child("messages").observe(.value, with: {(snapShots) in
            ...
            if battle == "しない" {
                //対戦をしない時の処理
                //ルームの削除
                self.ref.child("rooms").child(String(self.RoomID)).removeValue()
                print("ルームの削除")
                ...
                self.hazure = Int(arc4random_uniform(19))
                let enemyID = self.userDefault.string(forKey: "enemyID")
                
                //0なら自分からタップできる
                if self.num == 0 {
                    self.ref.child("rooms").child(String(self.RoomID)).child("battle").setValue(["Tap Player ID": self.user?.uid,"ハズレbutton": self.hazure])
                    
                    //1なら相手からタップできる
                }else if self.num == 1 {
                    self.ref.child("rooms").child(String(self.RoomID)).child("battle").setValue(["Tap Player ID": enemyID,"ハズレbutton": self.hazure])
                }
                
                //どちらがタップできるか取得
                self.ref.child("rooms").child(String(self.RoomID)).child("battle").observe(.childAdded, with: { [weak self](snapshot) -> Void in
                    ...
                    if Tap_Player_ID == useruid {
                        ...
                        self?.Playerlable.text = String(describing: enemyName)
                    }
                })
